...
float dco_LUT (float i)    /* contrast parameter */
...

{
    ...
    temp_result = (float)i;
    temp_result = pow(temp_result,0.2);
    temp_result = temp_result/15; // Valor já declarado do Labda
    if(temp_result != 0.0){
        temp_result = -(temp_result/5.0);
    }
    result = exp(temp_result);
...
for (i=1; i<=nx; i++)
 for (j=1; j<=ny; j++)

     {
       ...
       /* calculate weights */
    /*
       //qN  = LUT[fabs(g[i][j]-g[i  ][j+1])]; //(1.0 - exp(-8.0 * ht * dco(g[i][j], g[i  ][j+1], lambda))) / 8.0;
    ...
    */     
        ...
        aux = fabs(g[i][j]-g[i][j+1]);
        qN = LUT[aux];
        aux = fabs(g[i][j]-g[i+1][j+1]);
        qNE = LUT[aux];
        aux = fabs(g[i][j]-g[i+1][j]);
        qE = LUT[aux];
        aux = fabs(g[i][j]-g[i+1][j-1]);
        qSE = LUT[aux];
        aux = fabs(g[i][j]-g[i][j-1]);
...
void *diff2d_thread_half(void *threadarg){
    struct thread_args *my_data;
    my_data = (struct thread_args *) threadarg;

    for (int i=1; i<=my_data->imax; i++)
    {    
        diff2d (my_data->ht, my_data->lambda, (my_data->nx-my_data->indexMat), my_data->ny, my_data->f);
    }
    /*finaliza a função retornando o argumento que foi recebido*/
    pthread_exit(threadarg);
}
